...
export default async function PublicSite({ params }: { params: Promise<{ id: string[] }> }) {
  // 🧠 THE CATCH-ALL ROUTER LOGIC WITH NEXT 15/16 FIX
  const resolvedParams = await params;
  const siteSlug = resolvedParams.id[0];
  const pagePath = resolvedParams.id.length > 1 ? '/' + resolvedParams.id.slice(1).join('/') : '/';

  // 1. Fetch the entire site config
        ...
        {blocks.map((block: any) => {
            ...
            if (block.type === 'nav') return (
                <nav key={block.id} style={bgStyles} className={`px-6 md:px-12 py-5 flex justify-between items-center border-b border-opacity-20 z-50 shadow-sm shrink-0 ${borderClass}`}>
                    <a href={`/site/${siteSlug}`} className="flex items-center gap-3">
                    ...
                    <div className="hidden md:flex gap-8 text-sm font-bold opacity-80">
                        {block.data.links?.map((link:any, i:number) => {
                            // Link formatting logic so links work seamlessly!
                            const isAnchor = link.url.startsWith('#')
                            const finalUrl = isAnchor ? link.url : `/site/${siteSlug}${link.url.startsWith('/') ? link.url : '/' + link.url}`
